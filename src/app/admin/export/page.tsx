  ...
  const downloadExport = async (format: 'json' | 'csv') => {
    ...
    try {
      ...
      if (format === 'json') {
        ...
        a.href = url;
        a.download = `partner-export-${new Date().toISOString().split('T')[0]}.json`;
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `partner-export-${new Date().toISOString().split('T')[0]}.csv`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
      ...
      alert(`${format.toUpperCase()} 형식으로 내보내기가 완료되었습니다.`);
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600">파트너 및 평가 데이터 내보내기</p>
          </div>
          <button
            onClick={() => router.back()}
        ...
        <div className="bg-white p-6 rounded-2xl border shadow-sm">
          <h2 className="text-lg font-semibold mb-4">내보내기 형식 선택</h2>
          <p className="text-gray-600 mb-6">
            현재 모든 파트너 및 평가 데이터를 선택한 형식으로 내보낼 수 있습니다.
          </p>
          
          <div className="space-y-4">
            <div className="p-4 border rounded-lg">
              <h3 className="font-medium mb-2">내보내기 형식</h3>
              <p className="text-sm text-gray-600 mb-3">
                원하는 형식을 선택하여 데이터를 내보내세요.
              </p>
              <div className="flex gap-3">
                ...
                >
                  {exportLoading ? '내보내기 중...' : 'JSON 내보내기'}
                </button>
                <button
            ...
            <div className="p-4 border rounded-lg bg-blue-50">
              <h3 className="font-medium mb-2 text-blue-800">내보내기 정보</h3>
              <ul className="text-sm text-blue-700 space-y-1">
                <li>• 파트너 데이터: 모든 활성 파트너 정보</li>
                <li>• 평가 데이터: 모든 평가 기록</li>
